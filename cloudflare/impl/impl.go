...
func NewSite(varDomain string) (string, string, bool, error) {
    ...
    // 如果创建站点失败，则返回错误信息
    if !response.Success {
            return "", "", false, fmt.Errorf("failed to create site: %s", response.Errors[0].Message)
    }

    // 如果成功创建站点，则返回 NameServers
    return response.Result.NameServers[0], response.Result.NameServers[1], response.Success, nil
}
